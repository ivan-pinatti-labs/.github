...
container: every hook in `.pre-commit-config.yaml`, `gh`, and `git` pushes
over SSH. It is built on the organization's base image from
...
**This one does not nest containers.** Nothing in this repository's hooks
starts a container of its own, unlike the sibling repositories with
`hadolint-docker` or `actionlint-docker`, so it gets neither `/dev/fuse` nor
any of the nested networking arguments. The confined domain and category
below are here only so the container can reach the ssh-agent, which is a
different thing from nesting; see the run argument table.
...
| `label=type:container_engine_t` | Not for nesting, which this repository does not do. A container in the default `container_t` domain cannot connect to a socket served from `container_engine_t`, so without this the ssh-agent below is unreachable and `git push` over SSH fails. |
| `label=level:s0:c555,c666` | The same SELinux category as the ssh-agent container; a different category cannot connect to its socket either. |
| `--secret gh-devcontainer,...` | The GitHub token, as `GH_TOKEN`. |
| The workspace mount at the clone's own path | Paths inside the container match the host, so git worktree metadata resolves the same way in both. |
| The `devcontainer-ssh` mount | The ssh-agent socket and GitHub's host keys. |

There is deliberately no `--device /dev/fuse` and no nested networking here.
Those are what a repository whose hooks start containers needs, and adding
them to a repository that does not would widen what this container can do for
no reason.
