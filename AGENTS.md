...
The organization's special `.github` repository. There is no application
code, no build, no test suite and no merge queue here.

- `profile/README.md` renders on the organization's public GitHub page.
...
- `docs/crypto/addresses.md` and its QR codes hold the donation addresses the
  ...
  verify it against.
- `README.md` records the organization's dependency policy (Renovate and
  Dependabot schedules and the cooling window). Read it before changing bot
  configuration in any repository.
